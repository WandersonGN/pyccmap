...
def main():
    ...
    for exchange, data in directory.dict.items():
        ...
        for currency, info in data.items():
            print(f"{currency.ljust(4)}: " + "\t".join("= ".join((key.ljust(4), str(value).ljust(8))) for key, value in sorted(filter(info.items(), key = lambda x: x[0] in ("ask", "bid", "last")), key = lambda x: x[0])), end = ("\n" + (" " * 15)))
        print("")
